...
create policy if not exists "Anyone can read menu items" on public.menu_items
...
create policy if not exists "Users can manage own orders (select)" on public.orders
for select using (auth.uid() = user_id);
create policy if not exists "Users can manage own orders (insert)" on public.orders
for insert with check (auth.uid() = user_id);
create policy if not exists "Users can manage own orders (update)" on public.orders
for update using (auth.uid() = user_id);
create policy if not exists "Users can manage own orders (delete)" on public.orders
...
create policy if not exists "Users can read own order_items" on public.order_items
for select using (
  exists (select 1 from public.orders o where o.id = order_id and o.user_id = auth.uid())
);
create policy if not exists "Users can insert own order_items" on public.order_items
for insert with check (
  exists (select 1 from public.orders o where o.id = order_id and o.user_id = auth.uid())
);
create policy if not exists "Users can update own order_items" on public.order_items
for update using (
  exists (select 1 from public.orders o where o.id = order_id and o.user_id = auth.uid())
);
create policy if not exists "Users can delete own order_items" on public.order_items
for delete using (
  exists (select 1 from public.orders o where o.id = order_id and o.user_id = auth.uid())
);
...
create policy if not exists "Users can read own payments" on public.payments
for select using (
  exists (select 1 from public.orders o where o.id = order_id and o.user_id = auth.uid())
);
